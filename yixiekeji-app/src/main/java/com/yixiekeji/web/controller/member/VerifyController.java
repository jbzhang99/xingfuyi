...
import com.yixiekeji.core.StatusCode;
import com.yixiekeji.web.util.ValidateCodeUtils;
...
public class VerifyController {
    ...
    @ResponseBody
    public ServiceResult verify(HttpServletRequest request, HttpServletResponse response) throws IOException {
        //将随机数存在session中
        HttpSession session = request.getSession(true);
        ValidateCodeUtils.Validate v = ValidateCodeUtils.getRandomCode();     //直接调用静态方法，返回验证码对象
        if(v!=null) {
            session.setAttribute(ConstantsEJS.VERIFY_NUMBER_NAME, v.getValue().toUpperCase());    //将验证码值保存session
            return new  ServiceResult(true, StatusCode.OK, v.getBase64Str(), "刷新验证码成功！");
        }
        return new ServiceResult(false, StatusCode.ERROR,"刷新验证码失败!");
    }

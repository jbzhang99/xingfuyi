...
<div class='container'>
	<div class='breadcrumb'>
		...
		<span>
			&nbsp;>&nbsp;
			<a href='javascript:void(0)'>用户发货</a>
		</span>
	</div>
</div>
		
<form id ="productBackForm" action="${(domainUrlUtil.urlResources)!}/member/doBackDeliverGoods.html">
<!-- 隐藏域 -->
<input type="hidden" name="id" value="${(memberProductBack.id)!''}">
			...
			<div class='repair-steps'>
				<div class='repair-step repair-step-curr'>
					...
					<div class='sellerPrompt'>
						...
						<div class='fl'>
							<ul class='list-type list-type-new'>
								<li class='selected' >
									<a href='javascript:void(0);'>发货<b></b></a>
								</li>
							</ul>
						</div>
					</div>
					...
					<div class='sellerPrompt'>
						...
						<div class='fl'>
							<input type="text" name="logisticsNumber" id="logisticsNumber">
						</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		jQuery("#productBackForm").validate({
	        ...
	        rules : {
	            "logisticsNumber":{required:true,minlength:2,maxlength:255}
	        },
	        messages:{
	            "logisticsNumber":{required:"请输入快递单号"}
	        }
	    });
					...
					success:function(data){
						if(data.success){
							jAlert('保存成功', '提示',function(){
								window.location.href=domain+'/member/back.html'
							});
						}else{
							jAlert(data.message);

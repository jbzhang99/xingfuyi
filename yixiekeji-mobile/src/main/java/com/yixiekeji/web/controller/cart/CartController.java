...
public class CartController extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = "/buynow.html", method = { RequestMethod.POST })
    public @ResponseBody HttpJsonResult<Boolean> buynow(HttpServletRequest request,
        ...
        Product product = productResult.getResult();
        ProductGoods goods = null;
        if (productGoodId == null || productGoodId.intValue() == 0) {
            // 如果是货品是空，则可能是从列表页等不能选择货品的页面请求过来，随机取一个货品放入购物车
            ServiceResult<List<ProductGoods>> goodsResult = productGoodsService
        ...
        cart.setProductId(product.getId());
        cart.setSellerId(product.getSellerId());
        cart.setChecked(1); // m默认选中

        HttpJsonResult<Boolean> jsonResult = new HttpJsonResult<Boolean>();
        ServiceResult<Boolean> updatecount = cartService.updateCheckedAll(member.getId(), 0);

        if (!updatecount.getSuccess()) {
            if (ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR.equals(updatecount.getCode())) {
                throw new RuntimeException(updatecount.getMessage());
            } else {
                jsonResult = new HttpJsonResult<Boolean>(updatecount.getMessage());
            }
        } else {
            ServiceResult<Integer> serviceResult = cartService.addToCart(cart);
    ...
    public @ResponseBody HttpJsonResult<CartInfoVO> toMyCart(HttpServletRequest request,
                                                             HttpServletResponse response,
                                                             Map<String, Object> dataMap) {
    ...
    public @ResponseBody HttpJsonResult<CartInfoVO> getCartInfo(HttpServletRequest request,
                                                                HttpServletResponse response,
                                                                Map<String, Object> dataMap) {
        HttpJsonResult<CartInfoVO> jsonResult = new HttpJsonResult<CartInfoVO>();
        Member member = WebFrontSession.getLoginedUser(request);

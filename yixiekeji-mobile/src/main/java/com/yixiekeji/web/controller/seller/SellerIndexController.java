...
public class SellerIndexController extends BaseController {
    ...
    @RequestMapping(value = "/indexSeller.html", method = { RequestMethod.GET })
    public @ResponseBody HttpJsonResult<Seller> indexSeller(HttpServletRequest request,
    ...
    @RequestMapping(value = "/indexBannerSeller.html", method = { RequestMethod.GET })
    public @ResponseBody HttpJsonResult<List<MSellerIndexBanner>> indexBannerSeller(HttpServletRequest request,
                                                                                    HttpServletResponse response,
                                                                                    Map<String, Object> dataMap,
                                                                                    Integer sellerId) throws Exception {
        ...
        ServiceResult<List<MSellerIndexBanner>> bannerResult = mSellerIndexService
            .getMSellerIndexBannerForView(seller.getId(), true);

        if (!bannerResult.getSuccess()) {
            jsonResult.setMessage(bannerResult.getMessage());
        } else {
            jsonResult.setData(bannerResult.getResult());
        }
        return jsonResult;
    }

    @RequestMapping(value = "/indexFloorSeller.html", method = { RequestMethod.GET })
    public @ResponseBody HttpJsonResult<List<MSellerIndexFloor>> indexFloorSeller(HttpServletRequest request,
                                                                                  HttpServletResponse response,
                                                                                  Integer sellerId,
        ...
        ServiceResult<List<MSellerIndexFloor>> floorResult = mSellerIndexService
            .getMSellerIndexFloorsWithData(sellerId, true);
        if (!floorResult.getSuccess()) {
            jsonResult.setMessage(floorResult.getMessage());
        } else {
            jsonResult.setData(floorResult.getResult());
        }
        return jsonResult;
    }

    @RequestMapping(value = "/indexCollectedSeller.html", method = { RequestMethod.GET })
    public @ResponseBody HttpJsonResult<List<MemberCollectionSeller>> indexCollectedSeller(HttpServletRequest request,
                                                                                           HttpServletResponse response,
                                                                                           Integer sellerId,
                                                                                           Map<String, Object> dataMap) throws Exception {

        HttpJsonResult<List<MemberCollectionSeller>> jsonResult = new HttpJsonResult<List<MemberCollectionSeller>>();
        // 查询商户基本信息
        ServiceResult<Seller> sellerResult = sellerService.getSellerById(sellerId);
        if (!sellerResult.getSuccess()) {
            if (ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR.equals(sellerResult.getCode())) {
            ...
            if (collectionRlt.getResult() != null) {
                for (MemberCollectionSeller collectionSeller : collectionRlt.getResult()) {
                    if (collectionSeller.getState() != MemberCollectionSeller.STATE_2) {
                        collected = "true";
                        jsonResult.setData(collectionRlt.getResult());
                        break;
                    }
                }
            }

...
<#assign currentBaseUrl="${domainUrlUtil.urlResources}/admin/operate/courierCompany"/>
<style>
	.panel-fit body.panel-noscroll {
		overflow-y: scroll;
	}
</style>
<script language="javascript" src="${(domainUrlUtil.staticResources)!}/resources/admin/lodop/LodopFuncs.js"></script>
<script language="javascript" type="text/javascript"> 
    var LODOP; //声明为全局变量       
	function myBlankDesign() {   
		LODOP=getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
		LODOP.ADD_PRINT_SETUP_BKIMG("<img border='0' src='${(domainUrlUtil.imageResources)!}/${(obj.imagePath)!}'>");
		<#noescape>
		${(obj.content)!''}
		</#noescape>
		LODOP.PRINT_DESIGN();
...
<script language="javascript">
	$(function() {
		$("#back").click(function() {
			location.href = '${domainUrlUtil.urlResources}/admin/operate/courierCompany';
		});
	})
</script>
				...
				<dd class="dd-group">
					<div class="fluidbox">
						<label class="lab-item" style="width: 100%; text-align: left;">
						  ...
						  第一次使用需要安装浏览器插件 <a href="${(domainUrlUtil.staticResources)!}/resources/admin/lodop/Lodop.zip"><b style="color:red">下载</b></a> <br/>
						  ...
						  <img border='0' src='${(domainUrlUtil.staticResources)!}/resources/admin/lodop/1.jpg' /><br/>
						  ...
						  注意：其他信息不能录入，必须按照格式，否则打印不出来信息<br/>
						   <p>4、点击图上面三角图标，生成程序代码，如下如：</p>
						    <img border='0' src='${(domainUrlUtil.staticResources)!}/resources/admin/lodop/2.jpg' /><br/>
						    其中第一行和第二行数据不能选择，其他信息复制下来，然后返回到列表页，在模板内容选项中粘贴刚才复制的内容，如下如：<br/>
						    <img border='0' src='${(domainUrlUtil.staticResources)!}/resources/admin/lodop/3.jpg' /><br/>
						    点击保存模板录入完成，可以使用了
						  <br/>
						</label>
					</div>

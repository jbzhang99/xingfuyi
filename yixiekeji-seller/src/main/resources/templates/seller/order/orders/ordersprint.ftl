...
<style>
body {
	font: 14px/1.5 'Microsoft YaHei', "微软雅黑", Tahoma, Helvetica, Arial,
		sans-serif;
	margin: 0;
	padding: 0;
	border: 0;
...
body, div, dl, dt, dd, ul, ol, li, h1, h2, h3, h4, h5, h6, pre, form,
	fieldset, input, textarea, blockquote, p {
	padding: 0;
	margin: 0;
}
...
<script type="text/javascript"
	src="${(domainUrlUtil.staticResources)!}/resources/seller/jslib/js/jquery-1.8.2.min.js"></script>
<script type="text/javascript"
	src="${(domainUrlUtil.staticResources)!}/resources/seller/jslib/js/jquery.jqprint-0.3.js"></script>
<script>
	$(function() {
...
<body id="printArea">
	...
	<table width="80%" cellpadding="0" cellspacing="0" class="tab">
		...
		<tr>
			<td colspan="2">客户地址：${(orders.addressAll)!""} &nbsp;
				${(orders.addressInfo)!""}</td>
		</tr>
	</table>
	...
	<table width="80%" cellpadding="0" cellspacing="0" class="tab1">
		...
		<#if orders?? && orders.orderProductList?? &&
		orders.orderProductList?size &gt; 0 > <#list orders.orderProductList
		as op>
		<tr>
			<td>${(op.productId)!""}</td>
	...
	<div
		style="margin-top: 10px; text-align: right; width: 80%; margin: 20px auto auto">
		<#if orders?? && orders.isCodconfim?? && orders.isCodconfim == 1 >
		货到付款 &nbsp;&nbsp; </#if> 订单支付金额：<span>￥${(orders.moneyOrder -
			orders.moneyPaidBalance - orders.moneyIntegral)?string("0.00")!""}</span>
	</div>
</body>
</html>
...
public class CartController extends BaseController {
    ...
    * @Date: 2019/10/23 
    */
    @PostMapping(value = "/delCartByIds")
    @ResponseBody
    public ServiceResult delCartByIds(@RequestBody List<Integer> ids){
        if(ids != null){
            ServiceResult<Boolean> serviceResult = cartService.deleteCartByIds(ids);
            if(serviceResult.getResult()){
                return new ServiceResult(true, StatusCode.OK,"删除成功!");
            }
        }
        return new ServiceResult(false, StatusCode.ERROR,"删除失败!");
    }

   ...
   <!-- 头部 end-->
					...
					<dl class="flex list-dl">
						...
						<dd class="padl flex-2">
							...
							<div class="product-desript">
							   <p class="clr53">￥<font>${(producListVO.malMobilePrice)?string("0.00")!""}</font>
							...
							<#if producListVO.seller??><div class="shop_area">${(producListVO.seller)!}</div></#if>
						</dd>
					</dl>
					</a>
				</#list>
	...
	<div class="slidebar" id="slidebar">
		<div class="flex flex-align-center head-bar">
	   	  	 ...
	   	  	 <div class="flex-1 text-right"><a href="${(domainUrlUtil.urlResources)!}/cate/${(productCate.id)!''}.html">清除</a></div>
   	   </div>
...
function filterWhereAllNew(filter) {
	var urlPath = "${(urlPath)!}";
	self.location="${(domainUrlUtil.urlResources)!}/" + urlPath + filter + ".html";
}
...
$(function(){
	$("#product_list_more_json").click(function(){
		...
		var urljson = "${(domainUrlUtil.urlResources)!}/" + url + ".html";
            ...
            success:function(data){
                if (data.success) {
                    $.each(data.data, function(i, product){
                    	...
                    	listJsonHtml += "<div class='list-goods-tip'>";
                    	if(product.sellerId == 1) {
                    		listJsonHtml += "<span class='icon-group-1'>自营</span>"
                    	}
                    	if(product.coupon == 1) {
                    		listJsonHtml += "<span class='icon-group-4'>优惠券</span>"
                    	}
                    	if(product.special == 1) {
                    		listJsonHtml += "<span class='icon-group-3'>特卖</span>"
                    	}
                    	listJsonHtml += "</div>";
                    	listJsonHtml += "<div class='product-desript'>";
                    	listJsonHtml += "<p class='clr53'>￥<font>" + parseFloat(product.malMobilePrice).toFixed(2) + "</font>" 
                    	if(product.single == 1) {
                    		listJsonHtml += "<span class='icon-group-2'>立减</span>"
                    	}
                    	if(product.full == 1) {
                    		listJsonHtml += "<span class='icon-group-2'>满减</span>"
                    	}
                    	listJsonHtml += "</p>";
                    	...
                    	listJsonHtml += "&nbsp;&nbsp;";
                    	listJsonHtml += "<font>" + product.commentsNumber + "</font>条评价</p></div>";
                    	if(product.seller != "" && product.seller != "null") {
                    		listJsonHtml += "<div class='shop_area'>";
                    		listJsonHtml += product.seller;
                    		listJsonHtml += "</div>";
                    	}
                    	listJsonHtml += "</dd></dl></a>";
                    })

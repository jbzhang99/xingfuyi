...
<body class="bgf2">
   ...
   <header id="header">
   	  <div class="flex flex-align-center head-bar">
   	  	 ...
   	  	 <div class="flex-2 text-center">我的积分</div>
   ...
   <!-- 头部 end-->
	...
	<div class="s-containerr cl70">
	  ...
	  <div class="balance-content">	
		...
		<div id="integral_more"></div>
				
		<#if memberGradeIntegralLogss?? && memberGradeIntegralLogss?size == pagesize>
			<div id="integral_more_json" class="text-center font14 pad-top2">查看更多<i class="fa fa-angle-double-down"></i></div>
			<div id="integral_more_json_no" class="text-center font14 pad-top2" style="display:none;">已展示全部记录</div>
		<#else>
			<div id="integral_more_json_no" class="text-center font14 pad-top2">已展示全部记录</div>
		</#if>
...
$(function(){
	$("#integral_more_json").click(function(){
		var list_page = $("#list_page").val();
		list_page++;
		$("#list_page").val(list_page);
		
		var urljson = "${(domainUrlUtil.urlResources)!}/member/integralJson.html?pageNum=" + list_page;
		...
		$.ajax({
            ...
            success:function(data){
                if (data.success) {
                    ...
                    $("#integral_more").append(listJsonHtml);
                    if ((data.total) == ${(pagesize)!}) {
                        $("#integral_more_json").show();
                        $("#integral_more_json_no").hide();
                    } else {
                        $("#integral_more_json").hide();
                        $("#integral_more_json_no").show();
                    }
                }
            }
        });

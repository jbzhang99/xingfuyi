...
public class ProductNormModel {
    ...
    public Boolean saveNormMap(FeignProjectUtil feignProjectUtil) {
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRED);
        TransactionStatus status = transactionManager.getTransaction(def);
        try {

            ProductNorm norm = feignProjectUtil.getProductNorm();
            List<ProductNormAttr> attrs = feignProjectUtil.getProductNormAttrList();
    ...
    public Boolean updateNorm(FeignProjectUtil feignProjectUtil) {
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRED);
        TransactionStatus status = transactionManager.getTransaction(def);
        try {
            ...
            if (null != attrs && attrs.size() > 0) {
                for (int i = 0; i < attrs.size(); i++) {
                    Integer attrId = attrs.get(i).getId();
                    if (null != attrId) {
                        ProductNormAttr attr = productNormWriteDao.getNormAttrById(attrId);
                        if (null != attr) {
                            //update
                            productNormWriteDao.updateNormAttr(attrs.get(i));
                            attrNewIds.add(attrId);
                        }
                    } else {
                        //insert
                        attrs.get(i).setProductNormId(norm.getId());
                        this.dbConstrainsAttr(attrs.get(i));
                        productNormWriteDao.insertNormAttr(attrs.get(i));
                    }
                }
            }
